...
main :: proc() {
    ...
    up_err := update_todo(conn, &up_todo)
    if up_todo.description != nil {
        delete(up_todo.description^)
        free(up_todo.description)
    }
    if up_err != .None {
        log.errorf("Failed to update todo: %v", up_err)
        return
    }
    log.infof("Updated todo with ID: %d", new_id)

    all_todos: [dynamic]Todo
    get_all_err := get_todos(conn, &all_todos)
    if get_all_err != .None {
        log.errorf("Failed to get all todos: %v", get_all_err)
        return
    }
    log.info("All Todos:")
    for i := 0; i < len(all_todos); i+=1 {
        ...
        del_err := delete_todo(conn, todo.id)
        if del_err != .None {
            log.errorf("Failed to delete todo with ID %d: Error %v", todo.id, del_err)
        } else {
